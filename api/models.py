...
class User(models.Model):
    ...
    def add_payment(self, payment):
        last_payment_id = max(list(map(int, self.payments.keys()))) if len(self.payments.keys()) != 0 else 0
        self.payments.update({
            str(int(last_payment_id)+1): payment
        })
    ...
    def get_discord(self):
        ...
        """
        return str(self.discord) #if self.discord[0] == '@' else '@'+str(self.discord)
